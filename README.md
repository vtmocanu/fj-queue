...
Actions API on `git.example.com` (or any Forgejo host) and renders runner
...
Tracked under PRD [#61](https://github.com/vtmocanu/fj-queue/issues/2).
...
The default `--host` is `git.example.com`. Point at another Forgejo instance
...
| `--host HOST` | string, default `git.example.com` | Forgejo host. A bare hostname is auto-prefixed with `https://`; values that already start with `http://` or `https://` are accepted as-is. A malformed value (e.g. CRLF or other characters httpx rejects as `InvalidURL`) is rejected with exit 2 and a JSON error envelope on stdout. |
...
| `--metrics-url URL` | string, default `https://prometheus.example.com` | Prometheus base URL for the runner-metrics queries. Also settable via `$FJ_QUEUE_METRICS_URL` (the `--metrics-url` flag wins). No auth is sent to this endpoint. |
| `--metrics-namespace NS` | string, default `forgejo-runner` | Kubernetes namespace of the runner pods. |
| `--metrics-cluster {auto,green,blue}` | enum, default `auto` | Blue/green filter. Both clusters report `cluster="k8s-cluster"`, so the live one is told apart by node prefix (`k8s-node-*` vs `k8s-node-*`). `auto` applies no filter; `green` / `blue` keep only pods on that color's nodes. |
...
for resource usage is **Prometheus** (`https://prometheus.example.com`, no
...
- **Human formatting**: CPU is shown to 3 decimal cores (for example
  ...
  numbers.
- **Blue/green**: both clusters report `cluster="k8s-cluster"`, so the live
...
nix-cache.example.com) is actively serving nix packages or sitting idle, from
...
```bash
uv run fj_queue.py --format json --repo owner-c/my-app \
  | jq '.queue[0].position'
```
...
- PRD: [`prds/61-fj-queue.md`](https://github.com/vtmocanu/fj-queue/blob/main/prds/2-config-generalization.md) in `fj-queue` (full design rationale, scheduling-semantics source verification, milestones).
