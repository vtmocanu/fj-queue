...
**Priority**: Medium
**Status**: Complete. M1-M6 merged (PR [#6](https://github.com/vtmocanu/fj-queue/pull/6)) and released as v2.0.0 (full suite green, 327 passed). M7 history-scrub applied to the working tree, `main`, and the `v1.0.0`/`v2.0.0` tags (all verified clean); the copies retained in GitHub's `refs/pull/*` are a known accepted residual (not self-purgeable without deleting the repo or a GitHub Support request). Generalizes the tool (whose initial implementation and Prometheus/NCPS metrics already shipped) for public consumption.
